...
@st.cache_data
def load_and_prepare_data(zip_file_path):
    """
    This function now smartly opens the zip archive, finds the correct CSV,
    loads the data, and engineers all necessary features.
    """
    # This logic correctly handles the zip file both locally and on Streamlit Cloud
    with zipfile.ZipFile(zip_file_path, 'r') as z:
        # Find the actual CSV filename inside the zip, ignoring macOS metadata folders
        csv_filename = [name for name in z.namelist() if not name.startswith('__MACOSX/')][0]
        with z.open(csv_filename) as f:
            # We must specify only the columns we need to avoid errors
            columns_to_use = [
                'PREMISEADD', 'OWNERNAME', 'ADDRESS1', 'CITYSTZIP', 'SALEDATE',
                'DEEDDATE', 'TOTBALAMT', 'NEWTOTAL', 'OLDTOTAL', 'PROPTYPE'
            ]
            df = pd.read_csv(f, usecols=columns_to_use, low_memory=False)
...
if not high_tier_list.empty:
    num_results = st.sidebar.slider(
        'Number of High-Tier Results to Display', min_value=1, max_value=len(high_tier_list), value=min(100, len(high_tier_list))
    )
else:
    num_results = 0
...
if not high_tier_list.empty:
    ...
    def format_years_owned(y):
        ...
        return f"{y:.1f}"
    display_list.loc[:, 'YearsOwnedDisplay'] = display_list['YearsSinceLastSale'].apply(format_years_owned)
    st.dataframe(
